...
Chrome and ffmpeg are auto-downloaded to `~/.webreel` on first use, or you can set `CHROME_PATH`, `CHROME_HEADLESS_PATH`, and `FFMPEG_PATH`. Chrome launches with its sandbox enabled by default; set `WEBREEL_NO_SANDBOX=1` if your environment requires it disabled (for example, running as root in a container).
